...
def test_repair_bronze_years_is_a_noop_when_every_year_conforms(
    ...
    # Three years, 2006 deliberately wider:
    #  - a variable-delta pull taken for only some years legitimately leaves
    #    them wider, and a strict superset is not a deviation.
    #
    # Repair must not "fix" that by narrowing it to the modal shape.
    #
    # Two same-shape years so the modal set is a real vote rather than one
    # year agreeing with itself.
    #
    # Generated by Claude.
    data_root = tmp_path / "data"
    external_dir = data_root / "external" / "ipums"
    ...
    # appending fake manfest for two more years
    append_to_manifest(
        collection_dir,
        build_extraction_record(
            source="ipums_api",
